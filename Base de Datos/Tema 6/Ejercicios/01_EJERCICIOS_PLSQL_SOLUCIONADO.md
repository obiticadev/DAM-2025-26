...
Aquí tienes el código debidamente indentado y con **comentarios detallados** que explican la lógica de cada bloque, las reglas de negocio que aplican y el flujo de los datos.

### 1. Trigger de Integridad de Agentes
Este disparador actúa como una "llave de seguridad" para asegurar que cada agente esté correctamente asignado.
...
FOR EACH ROW
BEGIN
    -- REGLA 1: Un agente no puede quedar vacío (sin familia y sin oficina)
    IF (:new.familia IS NULL AND :new.oficina IS NULL) THEN
        RAISE_APPLICATION_ERROR(-20201, 'Un agente no puede ser huérfano');
        
    -- REGLA 2: Un agente no puede pertenecer a ambos sitios a la vez
...
### 2. Trigger de Histórico de Agentes
Este es un trigger de auditoría. Se encarga de registrar todos los cambios de los agentes en una tabla aparte (`histagentes`) para mantener un rastro de dónde ha estado cada agente.
...
DECLARE
    ...
    v_familia VARCHAR2(40);
    v_ahora   DATE := SYSDATE; -- Captura la fecha y hora actual
BEGIN
    -- CASO A: Se crea un nuevo agente
    IF INSERTING THEN
        -- Buscamos el nombre real de la familia o la oficina para guardarlo en el histórico
        IF (:new.familia IS NOT NULL) THEN
            SELECT nombre INTO v_familia FROM familias WHERE identificador = :new.familia;
            v_oficina := NULL;
        ELSE
            SELECT nombre INTO v_oficina FROM oficinas WHERE identificador = :new.oficina;
            v_familia := NULL;
        END IF;

        -- Insertamos el primer registro en el histórico (fecha_hasta queda NULL porque está activo)
        INSERT INTO histagentes (fecha_desde, fecha_hasta, identificador, nombre, familia, oficina)
        VALUES (v_ahora, NULL, :new.identificador, :new.nombre, v_familia, v_oficina);

    -- CASO B: El agente cambia de datos (ej: cambia de oficina o familia)
    ELSIF UPDATING THEN
        -- 1. Cerramos el registro anterior poniendo fecha actual en 'fecha_hasta'
        UPDATE histagentes 
        SET fecha_hasta = v_ahora 
        WHERE identificador = :old.identificador 
          AND fecha_hasta IS NULL;

        -- 2. Buscamos los nombres de los nuevos destinos
        IF (:new.familia IS NOT NULL) THEN
            SELECT nombre INTO v_familia FROM familias WHERE identificador = :new.familia;
            v_oficina := NULL;
        ELSE
        ...
        -- 3. Insertamos el nuevo registro con los datos actualizados
        INSERT INTO histagentes (fecha_desde, fecha_hasta, identificador, nombre, familia, oficina)
        VALUES (v_ahora, NULL, :new.identificador, :new.nombre, v_familia, v_oficina);

    -- CASO C: El agente es eliminado del sistema
    ELSIF DELETING THEN
        -- Simplemente cerramos el historial del agente indicando que terminó ahora
        UPDATE histagentes 
        SET fecha_hasta = v_ahora 
        WHERE identificador = :old.identificador 
          AND fecha_hasta IS NULL;
    ...
    -- NOTA: Se eliminaron los COMMIT ya que los triggers no pueden realizar COMMIT por sí mismos.
END;
/
```
...
### 3. Trigger de Control de Horario (Jornada)
Este disparador protege la tabla `familias` para que nadie pueda modificarla fuera de los días y horas permitidos.
...
BEGIN
    -- REGLA 1: No se permiten cambios los fines de semana
    -- TO_CHAR con 'DY' devuelve 'SAT' (Sábado) o 'SUN' (Domingo) en inglés
    IF (TO_CHAR(v_ahora, 'DY', 'NLS_DATE_LANGUAGE = ENGLISH') IN ('SAT', 'SUN')) THEN
        RAISE_APPLICATION_ERROR(-20301, 'No podemos manipular familias en fines de semana');
    END IF;

    -- REGLA 2: No se permiten cambios fuera del horario de 08:00 a 18:00
    -- TO_CHAR con 'HH24' devuelve la hora en formato 24h
    IF (TO_CHAR(v_ahora, 'HH24') < 8 OR TO_CHAR(v_ahora, 'HH24') >= 18) THEN
        RAISE_APPLICATION_ERROR(-20302, 'No podemos manipular familias fuera del horario de trabajo (08:00 - 18:00)');
    END IF;
END;
/
```
...
1.  **Indentación lógica:** Se aplicaron sangrías en bloques `IF`, `ELSIF` y dentro de las sentencias `SELECT` e `INSERT` para que se vea claramente qué depende de qué.
2.  **Eliminación de COMMITs:** En Oracle, un trigger no puede ejecutar un `COMMIT` o un `ROLLBACK` (esto daría error en tiempo de ejecución), ya que el trigger es parte de la transacción principal.
3.  **Corrección de la lógica de DELETE:** Se agrupó correctamente la lógica de borrado en el trigger histórico.
4.  **Uso de variables locales:** Se renombraron las variables locales (ej: `v_ahora`) para diferenciarlas claramente de los nombres de las columnas de las tablas.
5.  **Idioma en Fechas:** En el trigger de jornada, se añadió el parámetro de lenguaje para asegurar que el código funcione aunque el servidor de base de datos esté configurado en otro idioma.