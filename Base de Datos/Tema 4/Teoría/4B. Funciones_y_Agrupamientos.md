...
| **`NEXT_DAY(f, d)`**| Fecha del siguiente día _d_ (1=Lunes, 2=Martes...). | `NEXT_DAY(SYSDATE, 'LUNES')` |
| **`EXTRACT(campo FROM f)`**| Extrae el Año (YEAR), Mes (MONTH), o Día (DAY). | `EXTRACT(YEAR FROM SYSDATE)` ➔ **2024** |
...
*   `TO_NUMBER(cad, formato)`: Texto a Número.
    *   *Símbolos de formato*: `9` (posición numérica), `0` (relleno con cero), `$` (dólar), `L` (moneda local), `S` (signo +/-), `D` (punto decimal), `G` (separador de miles).
...
*   `TO_DATE(cad, formato)`: Texto a Fecha real.
    ...
    *   *Día*: `DD` (12), `DAY` (LUNES), `D` (Día de la semana 1-7).
    *   *Otros*: `Q` (Trimestre), `WW` (Semana del año), `HH24:MI:SS` (Hora completa).
