...
2. **Por configuración del mapper** (`@JsonInclude`, orden alfabético, módulos):
   ...
   Para ordenar las claves alfabéticamente a nivel global, la opción del reto es
   `mapper.configure(MapperFeature.SORT_PROPERTIES_ALPHABETICALLY, true)` (equivale
   a poner `@JsonPropertyOrder(alphabetic=true)` en cada clase, pero sin tocar los DTOs).
...
Pero registrar el módulo **no basta**: hay una segunda trampa que castiga el reto
de fechas. Con solo el `JavaTimeModule`, Jackson serializa por defecto las fechas
con `instante` como un **timestamp numérico** (`[2026,5,21]` para un `LocalDate`,
o un epoch como `1747785600.000000000` para un `Instant`). Para sacar el formato
**ISO-8601** legible (`"2026-05-21"`) hay que desactivar esa opción:

```java
mapper.registerModule(new JavaTimeModule());
mapper.disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS);   // ← clave
// ahora LocalDate.of(2026,5,21) -> "2026-05-21" (string ISO), no [2026,5,21]
```

Por eso el reto `serializarConFormatoFecha` falla si solo registras el módulo: el
test busca `"2026-05-21"` con `contains` y, sin desactivar `WRITE_DATES_AS_TIMESTAMPS`,
recibe el array `[2026,5,21]`. **Spring Boot ya desactiva esta opción por ti**, así
que en un controller las fechas salen ISO de fábrica; el tropiezo solo aparece
cuando construyes el `ObjectMapper` a mano.
...
**Cuidado con el `Double` y el dinero.** Que un decimal JSON aterrice como `Double`
es práctico para tipos genéricos, pero es una bomba si el número representa euros:
el `double` es binario y no puede representar exactamente `0.10`, así que sumas y
redondeos arrastran error (`0.1 + 0.2` da `0.30000000000000004`). Para importes
hay dos defensas:

- En un `Map<String, Object>` o un árbol genérico, dile a Jackson que lea los
  decimales como `BigDecimal` en lugar de `double`:
  `mapper.enable(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS)`.
- Mejor aún, en un DTO concreto declara el campo como `BigDecimal` (no `double`):
  Jackson respeta el tipo declarado y deserializa el decimal sin pérdida.

En este bloque los retos usan `record Precio(double valor)` y `record Dinero(double
cantidad, ...)` por simplicidad didáctica (y por eso `DineroSerializer` redondea con
`Math.round(cantidad*100)/100.0`), pero en una API de cobros de verdad el tipo
correcto es `BigDecimal`.
...
Trampa de alcance: un serializer registrado por **tipo** aplica a TODAS las
instancias de ese tipo en ese mapper. Registrar uno para `Map.class` (reto 7)
cambia cómo ese mapper serializa *cualquier* `Map`, no solo el del reto; por eso
esos retos usan un `ObjectMapper` local y de usar y tirar, en vez del `MAPPER`
global del fichero. Ensuciar el mapper compartido haría fallar tests vecinos.
...
| 4 | Registrar el módulo pero ver `[2026,5,21]` en vez de `"2026-05-21"` | `disable(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS)` (Spring ya lo hace) |
| 5 | El JSON trae un campo extra y Jackson revienta | `configure(FAIL_ON_UNKNOWN_PROPERTIES, false)` |
| 6 | `node.get("x")` y NPE al encadenar | Usa `path("x")`, que devuelve nodo missing, no null |
| 7 | Comillas simples o claves sin comillas en JSON de prueba | JSON solo admite comillas dobles, también en las claves |
| 8 | En un serializer usar `writeNumber` cuando el test espera string | El test compara `"9.90 €"` con comillas → `writeString` |
| 9 | Esperar `Long` al deserializar un número a `Map<String,Object>` | Un entero JSON llega como `Integer`, un decimal como `Double` |
| 10 | Usar `double` para dinero y perder precisión al deserializar | `BigDecimal` en el DTO o `DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS` |
| 11 | Modificar el `JsonNode` raíz sin castear a `ObjectNode` | `put`/`remove` solo existen en `ObjectNode` |
| 12 | Asumir que `record` no se serializa (intentar getters manuales) | Jackson usa los accesores del record de fábrica |
| 13 | Registrar un serializer de `Map.class` en el mapper compartido | Usa un `ObjectMapper` local; afecta a TODOS los `Map` de ese mapper |
...
Fechas       = JavaTimeModule + disable(WRITE_DATES_AS_TIMESTAMPS) → ISO "2026-05-21" (Spring ya lo hace)
Dinero       = NUNCA double · BigDecimal en el DTO o USE_BIG_DECIMAL_FOR_FLOATS
...
7. ¿Qué DOS cosas hacen falta para que un `LocalDate` salga como `"2026-05-21"`
   (ISO) y no como `[2026,5,21]`? ¿Cuál de ellas hace Spring Boot por ti? *(2.3)*
8. Al deserializar un número JSON a `Map<String,Object>`, ¿de qué tipo Java llega
   un `25`? ¿Y un `3.14`? ¿Por qué ese `Double` es un problema si es dinero, y cómo
   lo evitas? *(2.4)*
</content>
</invoke>
