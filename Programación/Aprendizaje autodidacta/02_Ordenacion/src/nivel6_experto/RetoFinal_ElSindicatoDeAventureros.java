...
public class RetoFinal_ElSindicatoDeAventureros {
    ...
    public static void ejercicio() {
         ...
         * 4. Usa un método intermedio de stream que elimine Copias Mágicamente (Pista:
         * lo hace mirando Equals/Hashcode).
         * 5. Filtra para quedarte SÓLO con la clase "Mago".
         * 6. Filtra para quedarte SÓLO con los vivos.
         * 7. ORDENACIÓN MULTICRITERIO SÓLIDA:
         * - Principalmente, ordénalos por SU ORO de forma DESCENDENTE (El que más oro
         * tenga vaya el primero).
         ...
         * donde la Llave (Key) sea el Nombre, y el Valor (Value) sea la cantidad
         * íntegra de ORO ganado.
         * =========================================================================
         */

        Map<String, Double> mapaEliteMagos = null; // <- ESCRIBE AQUÍ TU MEGAPIPELINE (.stream().filter()...)
        mapaEliteMagos = baseDatosBruta.stream()
                .filter(a -> a != null)
                .filter(a -> a.getNivel() > 0)
                .distinct()
                .filter(a -> "Mago".equals(a.getClaseClase()))
                .filter(Aventurero::isEstadoVivo)
                .sorted(Comparator.comparing(Aventurero::getOro).reversed().thenComparing(Aventurero::getNivel)
                        .reversed())
                .collect(Collectors.toMap(Aventurero::getNombre, Aventurero::getOro));
        ...
        if (mapaEliteMagos != null) {
            ...
            if (tamañoCorrecto && contieneAMerlinYVivi && merlinTieneMasOroQueViviEnValores) {
                System.out.println(
                        ">> ¡[ÉXITO TOTAL]! Has demostrado una comprensión Nivel 100 de Mapeo, Tratamiento Nulo, Filtrado Complejo y Recolección Estructural.\033[0;32m [OK]\033[0m");
                ...
                System.err.println(
                        "2. Los valores del Diccionario sean dobles que registren su oro exacto (Ej: Vivi -> 1200.0).");
            }
        } else {
