...
public class Plataforma {
    ...
    public StringBuilder listarValoracionMedia(){
        StringBuilder salida = new StringBuilder();
        for (int i = 0; i < juegos.length; i++) {
            salida.append(i+1).append(". ").append(juegos[i]).append(": ").append(indiceValoracionMedia[i]).append("\n");
        }
        return salida;
    }
    ...
    public boolean validarJuegoSeleccionado(int entrada){
        entrada--;
        if (entrada >= 0 && entrada < juegos.length) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean validarPuntuacion(int entrada){
        if (entrada >= PUNTUACION_MIN && entrada <= PUNTUACION_MAX ) {
            return true;
        } else {
            return false;
        }
    ...
    public void puntuarJuego(int juegoSeleccionado, int puntuacion){
        ...
        indiceValoracionMedia[juegoSeleccionado] = (double)(indiceConPuntuacion[juegoSeleccionado])/(double)(indiceConContador[juegoSeleccionado]);
    }
